...
compscheck <- unique(c(comps$c1, comps$c2))
...
sobjlist <- lapply(sobjlist, function(sobj){
  ...
  #Nov 14 2024: check if objects already have "percent.mito" and "percent.hemo" and "Phase", thus allowing user to pass these if using seurat objects as input
...
af_md_list <- foreach(code = sample_metadata$Code,
                                    ...
                                    'scDAPP','grid', 'irlba', 'Matrix'), .verbose = T) %dopar%
  {
    ...
    # as of Aug 4th, we set this INSIDE the loop, since future seems to throw various errors... unfortunate
    options(future.globals.maxSize = 15000 * 1024^2) # 15 GB 
    ...
    # sobj <- Seurat::SCTransform(sobj, verbose = F)
    # UPDATE AGAIN AUG 12; glmgampoi did not seem to cause the memory problem
    ...
    if(doubletFinder == T){
      ...
      #as of Aug 3 2024, turn off glmGamPoi due to error related to future global maxsize??
      # sobj_df <- Seurat::SCTransform(sobj_df, verbose = F)
      # UPDATE AGAIN AUG 12; glmgampoi did not seem to cause the memory problem
    ...
    #prep genes
    if(length(levels(sobjraw$seurat_clusters)) > 1){
      n <- 5
...
RISC requires the selection of a reference sample from among the input datasets. The optimal reference sample is one that has the most diversity of celltypes.
...
if(run_ORA == T){
  ...
  ORA_mainlist_comps <- ORA_crosscondition_module(m_bycluster_crosscondition_de_comps = m_bycluster_crosscondition_de_comps,
                                                  pathways = pathways,
                                                  sample_metadata = sample_metadata,
                                                  comps = comps,
                                                  pathway_padj_thres = pathway_padj_thres,
