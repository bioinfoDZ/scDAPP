...
# code for risc_reference works best if its set to NULL rather than "auto" string
if(!is.null(risc_reference)){
  if(risc_reference == 'auto'){ risc_reference <- NULL }
}
...
af_md_list <- foreach(code = sample_metadata$Code,
  ...
  res <- scDAPP::per_sample_qc_module(
    ...
    rawobj_path = paste0(rawobjsdir, '/Unfiltered-SeuratObject-', code, '.rds'),
...
# Save per-sample resume cache (QC report + post-processing objects) after first successful run
```{r resume_save_samples, results='hide', message=TRUE}
...
if (is.null(risc_reference) || risc_reference == "auto") {
  risc_reference <- NULL
}
...
maxautoscore <- if (!is.null(refscore) && length(refscore)) {
...
Automatic tuning selected **%d** principal components and Louvain resolution **%s** (parameter id `%s`) by maximizing a combined bootstrap stability score: `0.5 * mean ARI + 0.5 * mean per-cluster Jaccard`. Each grid point is labeled like `PCs_1-N.res_R`, meaning the first N integration PCs and Louvain resolution R. Full tables and PDF copies of these plots are in `%s`.
...
cat('RISC requires the selection of a reference sample from among the input datasets. The optimal reference sample is one that has the most diversity of celltypes.


')

plotlab <- '
One way to select the reference sample is manual inspection of the "InPlot". This plot consists of three panels. The top panel is the most important and shows the number of clusters detected in each sample across a range of different PC values. Essentially, this is a proxy of the number of cell types detected in the dataset, and the sample with most clusters should be selected as the reference. The second panel allows selection of the optimal PC value to use and the amount of variance each PC has for each sample, with good samples having a higher score (indicating more diversity). The third panel shows distributional divergence of the samples, where a high score is "bad" and indicates that the sample is potentially composed of quite different celltypes from the rest of the dataset, and can potentially indicate some issue in data generation, for example that some incorrect tissue was sampled during data generation.
...
cat(sprintf(plotlab, maxautoscore))

refscoredf <- data.frame(
  Code = names(refscore),
  RefScore = as.numeric(refscore),
  stringsAsFactors = FALSE
)
refscoredf$Max <- ''
refscoredf$Max[refscoredf$Code == maxautoscore] <- '*'
refscoredf$RefScore <- round(refscoredf$RefScore, 5)
refscoredf$Set <- paste0("Set-", seq_len(nrow(refscoredf)))
...
for (i in seq_along(twt_samp_to_clust_list)) {
  name <- names(twt_samp_to_clust_list)[i]
  plotlab <- "
...
"
  ...
  print(summaryplots_integrated$indi_int_clust_twt_list[[i]])
